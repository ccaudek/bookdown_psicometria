# Statistica descrittiva {#chapter:descriptive_stats}
...
## Distribuzioni di frequenze {#sec:distr_freq}
...
$[a_i, b_i)$ o aperto a sinistra $(a_i, b_i]$[^1]. Ad ogni classe
...
Poniamoci ora il problema di costruire la tabella precedente partendo dai dati grezzi messi a disposizione da @zetsche_future_2019. Iniziamo a caricare i pacchetti che ci servono.
...
Ci sono dunque `r length(unique(bysubj$esm_id))` soggetti i quali hanno ottenuto i valori sulla scala del BDI-II stampati di seguito (li mostriamo ordinati dal più piccolo al più grande).

```{r}
sort(bysubj$bdi)
```

Calcoliamo ora le frequenze assolute per i seguenti intervalli aperti a destra: [0, 13.5), [13.5, 19.5), [19.5, 28.5), [28.5, 63). Esaminando i dati, vediamo che 36 soggetti cadono nella prima classe. Dobbiamo però eseguire quest'operazione di conteggio utilizzando R. 
...
Come si trova l'altezza delle barre dell'istogramma? Per la classe [0, 13.5), ad esempio, la frequenza relativa è 36/66. Tale valore corrisponde all'__area__ del rettangolo.  Dato che la base del rettangolo è 13.5, l'altezza sarà 36/66 / 13.5, ovvero {r 36/66 / 13.5}. E così via per le altre barre dell'istogramma.

Una rappresentazione grafica dell'istogramma delle frequenze relative si può ottenere con R utilizzando le funzioni di `ggplot2`.
...
```{r hist1zetsche, fig.cap="Istogramma per i valori BDI-II riportati da @zetsche_future_2019."}
p1 <- bysubj %>%
  ggplot(aes(x = bdi)) +
  geom_histogram(
    aes(y = ..density..),
...
```{r hist2zetsche, fig.cap="Una rappresentazione più comune per l'istogramma dei valori BDI-II di @zetsche_future_2019 nella quale gli intervalli delle classi hanno ampiezze uguali."}
p2 <- bysubj %>%
  ggplot(aes(x = bdi)) +
  geom_histogram(
    aes(y = ..density..),
...
```{r zetschehist3, fig.cap="Funzione di densità empirica per i valori BDI-II di @zetsche_future_2019."}
p3 <- bysubj %>% 
  ggplot(aes(x = bdi)) +
  geom_histogram(
    aes(y = ..density..), 
...
```{tikz, echo=FALSE, fig.cap = "1: Asimmetria negativa. 2: Asimmetria positiva. 3: Distribuzione unimodale. 4: Distribuzione bimodale.", fig.ext = 'png', cache=TRUE}
\usetikzlibrary{
...
figura [\[fig:boxplot\]](#fig:boxplot){reference-type="ref"
reference="fig:boxplot"}). Il valore adiacente superiore è il valore più
grande tra le osservazioni che risulta minore o uguale a $Q_3+1.5$ IQR.
I valori esterni ai valori adiacenti (chiamati *valori anomali*) vengono
rappresentati individualmente nel box-plot per meglio evidenziarne la
presenza e la posizione.

Consideriamo ora un caso concreto nel quale viene utilizzato un
box-plot. Nel caso dei dati di @zetsche_future_2019 riportati nella
tabella [1.1](#tab:Zetsche2020){reference-type="ref"
reference="tab:Zetsche2020"} ci chiediamo in che modo si differenziano
le distribuzioni del BDI-II tra i due gruppi considerati, ovvero tra il
gruppo dei pazienti e il gruppo di controllo. La
figura [1.3](#fig:Zetsche_violin_plot){reference-type="ref"
reference="fig:Zetsche_violin_plot"} ci fornisce due rappresentazioni
grafiche possibili che possono essere utilizzate per rispondere a questa
domanda.

![Due versioni di un "violin plot" dei valori BDI-II per ciascun gruppo
specificato nella tabella [1.1](#tab:Zetsche2020){reference-type="ref"
reference="tab:Zetsche2020"}.](Zetsche_violin_plot.pdf){#fig:Zetsche_violin_plot
width="1.0\\linewidth"}

Nella figura [1.3](#fig:Zetsche_violin_plot){reference-type="ref"
reference="fig:Zetsche_violin_plot"} sinistra sono rappresentati i dati
...
figura [1.3](#fig:Zetsche_violin_plot){reference-type="ref"
reference="fig:Zetsche_violin_plot"} destra sono rappresentanti gli
...
mediana di una quantità maggiore di 1.5 IQR.
...
tabella [1.1](#tab:Zetsche2020){reference-type="ref"
reference="tab:Zetsche2020"} di quanto lo faccia un istogramma. Inoltre,
...
figura [1.3](#fig:Zetsche_violin_plot){reference-type="ref"
reference="fig:Zetsche_violin_plot"}.
...
Nell'eq. [\[eq:media_aritm\]](#eq:media_aritm){reference-type="eqref"
reference="eq:media_aritm"} abbiamo usato la notazione delle sommatorie
...
cioè $$\sum_{i=1}^n (x_i - \bar{x}) = 0.\notag
\label{eq:diffmeansumzero}$$ Infatti, $$\begin{aligned}
\sum_{i=1}^n (x_i - \bar{x}) &= \sum_i x_i - \sum_i \bar{x}\notag\\
&= \sum_i x_i - n \bar{x}\notag\\
&= \sum_i x_i - \sum_i x_i = 0.\notag\end{aligned}$$ Ciò ci consente di
pensare alla media come al baricentro della distribuzione.
...
figura [1.3](#fig:Zetsche_violin_plot){reference-type="ref"
reference="fig:Zetsche_violin_plot"}. In tali circostanze, la tendenza
centrale della distribuzione è meglio rappresentata dalla mediana o
dalla media spuntata.
...
[\[eq:media_aritm\]](#eq:media_aritm){reference-type="eqref"
reference="eq:media_aritm"} dei dati rimanenti.
...
medio semplice dalla mediana* è la quantità
...
dall'eq. [\[eq:scostam_medio_sempl\]](#eq:scostam_medio_sempl){reference-type="eqref"
reference="eq:scostam_medio_sempl"} è molto intuitiva, la misura di
...
\label{eq:var_descr}$$ è la media dei quadrati degli scarti
$x_i - \bar{x}$ tra ogni valore e la media della distribuzione. La
...
originari e quindi non consente un'interpretazione intuitiva.

### Deviazione standard

Per tali ragioni la misura più usata della dispersione di una
distribuzione di dati è la *deviazione standard*, ovvero la radice
...
nel caso della media $\bar{x}$, anche la deviazione standard è
fortemente influenzata dai dati anomali (*outlier*), ovvero dalla
...
osservazioni sono uguali, $s=0$, altrimenti $s > 0$.

Alla deviazione standard può essere assegnata una semplice
interpretazione: la deviazione standard è *simile* (anche se non uguale)
...
@zetsche_future_2019, ciascuna osservazione dista, in media, 5.33 punti
...
pensare che, all'incirca, essa rappresenti una tale distanza media.
...
l'*affidabilità* degli strumenti di misurazione è minore che in altre
...
stesso costrutto[^3], allora i valori prodotti dalle due scale dovranno
...
figura [1.4](#fig:Zetsche_scatterplot){reference-type="ref"
reference="fig:Zetsche_scatterplot"}, cascun punto corrisponde ad un
...
![Associazione tra le variabili BDI-II e CES-D nel campione esaminato da
@zetsche_future_2019. Ai margini del diagramma a dispersione sono
rappresentate le funzioni di densità empirica delle due
variabili.](Zetsche_scatterplot.pdf){#fig:Zetsche_scatterplot
width="80%"}

Dalla figura [1.4](#fig:Zetsche_scatterplot){reference-type="ref"
reference="fig:Zetsche_scatterplot"} possiamo vedere che i dati mostrano
...
L'eq. [\[eq:definition_covariance\]](#eq:definition_covariance){reference-type="eqref"
reference="eq:definition_covariance"} ci fornisce dunque la definizione
della covarianza.

Per capire il significato
dell'eq. [\[eq:definition_covariance\]](#eq:definition_covariance){reference-type="eqref"
reference="eq:definition_covariance"}, supponiamo di dividere il grafico
della figura [1.4](#fig:Zetsche_scatterplot){reference-type="ref"
reference="fig:Zetsche_scatterplot"} in quattro quadranti definiti da
...
$$r_{XY} = \frac{S_{XY}}{s_X s_Y}.$$ La quantià che si ottiene in questo
modo viene chiamata *correlazione* di Bravais-Pearson (dal nome degli
autori che, indipendentemente l'uno dall'altro, la hanno introdotta).
...
Per i dati della
figura [1.4](#fig:Zetsche_scatterplot){reference-type="ref"
reference="fig:Zetsche_scatterplot"}, la covarianza è 207.426. Il segno
...
Facendo riferimento nuovamente alla
figura [1.4](#fig:Zetsche_scatterplot){reference-type="ref"
reference="fig:Zetsche_scatterplot"}, possiamo dire che, in molte
applicazioni (ma non nel caso presente!) l'asse $x$ rappresenta una
...
psicometrico è la *validità divergente*: i risultati di test
...
figura [1.5](#fig:Cairo_zero_corr){reference-type="ref"
reference="fig:Cairo_zero_corr"}.

![Due insiemi di dati (fittizi) per i quali i coefficienti di
correlazione di Pearson sono entrambi 0. Ma questo non significa che non
vi sia alcuna relazione tra le
variabili.](Cairo_zero_corr){#fig:Cairo_zero_corr width="80%"}
...
[^1]: Un intervallo aperto non include i suoi estremi ed è indicato con
    ...
    intervalli aperti, "$(a, b)$", intervalli chiusi, "$[a, b]$",
    intervalli aperti a sinistra, "$(a, b]$", e intervalli aperti a
    destra, "$[a, b)$".

[^2]: Si noti che solitamente i software restituiscono un valore
    *interpolato* di $p$-esimo quantile $q_p$ $(0 < p < 1)$, il quale
...
[^3]: Per costrutto si intende un concetto astratto non direttamente
