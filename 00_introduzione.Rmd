...
Vi posso dire cosa faccio io, quando devo affrontare problemi simili a quelli che dovrete affrontare voi, quando tali problemi sono nuovi per me. Il processo che seguo è simile a quello illustrato nella \@ref(fig:whattodo) la quale, ovviamente, è ironica, dato che fa uso di un "oggetto statistico" che impareremo a conoscere molto bene.  Ma la curva della figura \@ref(fig:whattodo), qui, è semplicemente la rappresentazione di una collina (ostacolo) che dobbiamo superare.  La prima fase dello studio (nella figura, la salita), quando dobbiamo risolvere un problema, è quella in cui non sappiamo come procedere perché non sappiamo come si può arrivare alla soluzione partendo dai dati del problema.  Questa è la parte più faticosa del processo: è necessario capire cosa ci chiede il problema e cosa dobbiamo fare per risolverlo.  In questa fase è necessario "capire", ovvero mettere in relazione le nozioni astratte che abbiamo studiato (e che dobbiamo conoscere, altrimenti... non c'è soluzione) e le caratteristiche specifiche dell'esercizio.  Una volta compresa la relazione che intercorre tra le informazioni dell'esercizio e i concetti astratti della teoria statistica che sono pertinenti in quel contesto, dobbiamo ancora  capire come si può usare un software (R) per applicare gli strumenti teorici appropriati alla situazione specifica. Questo secondo obiettivo è molto più facile del primo (siamo nella fase di "discesa" rappresentata nella figura).  E, ricordatevi: "Google is your friend!".

```{r whattodo, fig.align = 'center', out.width = "80%", fig.cap = "Che fare?", echo = FALSE}
knitr::include_graphics(here::here("images", "what_to_do.png"))
```
