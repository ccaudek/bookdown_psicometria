...
figura [1.1](#fig:demere){reference-type="ref" reference="fig:demere"}.
...
3.  se $A_1, A_2, \dots, A_k$ sono eventi disgiunti, allora la
    ...
    loro separate probabilità: $P(A_1 \text{ o } A_1 \dots \text{ o } A_k) = P(A_1) + P(A_2) +\dots + P(A_k).$
...
l'eq. [\[eq:kolmogorov_3\]](#eq:kolmogorov_3){reference-type="eqref"
reference="eq:kolmogorov_3"}.

[\[exercise:prob_distr_2dice\]]{#exercise:prob_distr_2dice
label="exercise:prob_distr_2dice"} Consideriamo nuovamente lo spazio
campionario $\Omega$
dell'esercizio [\[exercise:sampling_space_2dice\]](#exercise:sampling_space_2dice){reference-type="ref"
reference="exercise:sampling_space_2dice"} e definiamo la variabile
aleatoria $S(\omega)$ come la somma dei puntini che si ottengono dal
lancio di due dadi. Per esempio,
$S\big(\epsdice{6}\epsdice{3}\big) = 6+3=9$. Iniziamo a chiederci qual è
la probabilità dell'evento $S = 7$.

Per risolvere tale problema iniziamo a considerare il fatto che l'evento
$S = 7$ si verifica in corrispondenza di sei punti elementari dello
spazio campionario $\Omega$: $\epsdice{1}\epsdice{6}$,
$\epsdice{2}\epsdice{5}$, $\epsdice{3}\epsdice{4}$,
$\epsdice{4}\epsdice{3}$, $\epsdice{5}\epsdice{2}$,
$\epsdice{6}\epsdice{1}$. Dunque,
...
evento elementare dello spazio campionario ha probabilità $\frac{1}{36}$
e la probabilità cercata diventa $\frac{1}{6}$. È facile estendere il
ragionamento fatto sopra a tutti i valori che $S$ può assumere. In
questo modo giungiamo alla funzione di massa di probabilità $P_0$
riportata nella prima riga della
tabella [1.1](#tab:massa_prob_due_dadi_on_tr){reference-type="ref"
reference="tab:massa_prob_due_dadi_on_tr"}.

::: {#tab:massa_prob_due_dadi_on_tr}
<table>
<caption>Distribuzione di massa di probabilità per la somma dei punti prodotti dal lancio di due dadi bilanciati (<span class="math inline"><em>P</em><sub>0</sub></span>) e di due dadi truccati (<span class="math inline"><em>P</em><sub>1</sub></span>).</caption>
<thead>
<tr class="header">
...
<tbody>
<tr class="odd">
<td style="text-align: center;"><span class="math inline"><em>P</em><sub>0</sub>(<em>S</em> = <em>s</em>)</span></td>
...
<p><span class="math inline"><em>P</em><sub>1</sub>(<em>S</em> = <em>s</em>)</span></p></td>
<td style="text-align: center;"><span class="math inline">$\frac{4}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{4}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{5}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{6}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{7}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{12}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{7}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{6}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{5}{64}$</span></td>
<td style="text-align: center;"><span class="math inline">$\frac{4}{64}$</span></td>
...
: Distribuzione di massa di probabilità per la somma dei punti prodotti
dal lancio di due dadi bilanciati ($P_0$) e di due dadi truccati
($P_1$).
:::

[\[tab:massa_prob_due_dadi_on_tr\]]{#tab:massa_prob_due_dadi_on_tr
...
\label{eq:loaded_dice}\end{aligned}$$ Nel caso dei due dadi truccati, la
probabilità dell'evento elementare $\epsdice{1}\epsdice{1}$ è
$\frac{1}{4}\frac{1}{4}$. Dunque, $P(S=2) = \frac{4}{64}$. La
probabilità dell'evento elementare $\epsdice{1}\epsdice{2}$ è
$\frac{1}{4}\frac{1}{8}$. Tale valore è uguale alla probabilità
dell'evento elementare $\epsdice{2}\epsdice{1}$. Dunque, la probabilità
che $S$ sia uguale a 3 è
$\frac{1}{4}\frac{1}{8} + \frac{1}{8}\frac{1}{4} = \frac{4}{64}$, e così
via. Svolgendo i calcoli per tutti i possibili valori di $S$ otteniamo
la funzione di massa di probabilità $P_1$ riportata nella seconda riga
della tabella [1.1](#tab:massa_prob_due_dadi_on_tr){reference-type="ref"
reference="tab:massa_prob_due_dadi_on_tr"}.

Si noti che, a partire dalla funzione di massa di probabilità di $S$, è
possibile calcolare la probabilità di altri eventi. Per esempio,
possiamo dire che l'evento $S > 10$ ha una probabilità minore nel caso
dei dadi bilanciati, $P_0(S > 10) = \frac{3}{36} = \frac{1}{12}$,
rispetto al caso dei dadi truccati considerati in precedenza,
$P_1(S > 10) = \frac{8}{64} = \frac{1}{8}$.
