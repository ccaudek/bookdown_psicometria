...
Sia $\{H_1, H_2, H_3\}$ una partizione dello spazio campionario
...
e $E \cap H_3$, allora la probabilità $P(E)$ è data dalla somma delle
probabilità di questi tre eventi. Ciò è illustrato nella figura seguente.
...
\tikzset{
    ...
    \draw[H] ($(left.north west) ! .4 ! (left.north east)$) -- ($(left.south west) ! .35 ! (left.south east)$);
    \draw[H] ($(left.north west) ! .6 ! (left.north east)$) -- ($(left.south west) ! .66 ! (left.south east)$);
    ...
    \draw[H] ($(bottom.north west) ! .4 ! (bottom.north east)$) -- ($(bottom.south west) ! .35 ! (bottom.south east)$);
    \draw[H] ($(bottom.north west) ! .6 ! (bottom.north east)$) -- ($(bottom.south west) ! .66 ! (bottom.south east)$);
...
Sia $\{H_1, H_2\}$ una partizione dello spazio campionario $\Omega$.
Consideriamo un terzo evento $E \subset \Omega$ con probabilità non
nulla di cui si conoscono le probabilità condizionate rispetto ad $H_1$
e a $H_2$, ovvero $P(E \mid H_1)$ e $P(E \mid H_2)$. È chiaro per le
ipotesi fatte che se si verifica $E$ deve anche essersi verificato
almeno uno degli eventi $H_1$ e $H_2$. Supponendo che si sia verificato
l'evento $E$, ci chiediamo: qual è la probabilità che si sia verificato
$H_1$ piuttosto che $H_2$?
...
\end{equation}
Notiamo che la probabilità riportata al numeratore, ovvero $P(E \cap H_1)$, può essere ricavata dalla formula seguente:
\begin{equation}
P(E \mid H_1) = \frac{P(E \cap H_1)}{P(H_1)},\notag
\end{equation}
ovvero,
\begin{equation}
P(E \cap H_1) = P(E \mid H_1) P(H_1).\notag
\end{equation}
Sostituendo, otteniamo
\begin{equation}
P(H_1 \mid E) = \frac{P(E \mid H_1)P(H_1)}{P(E)}.\notag
\end{equation}
Inoltre, sapendo che $E = (E \cap H_1) \cup (E \cap H_2)$ e che $H_1$ e $H_2$ sono eventi
disgiunti, ovvero $H_1 \cap H_2 = \emptyset$, ne segue che possiamo calcolare $P(E)$ utilizzando il teorema della probabilità totale:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Sostituendo il risultato precedente nella formula della probabilità condizionata $P(H_1 \mid E)$ otteniamo:
\begin{equation}
...
Siano $H_1$, $H_2,$ ..., $H_n$ $n$ eventi disgiunti con $P(H_i) > 0$ e
tali che $\bigcup_{i=1}^{n} H_i = \Omega$. Per l'evento $E \subset \Omega$ con $P(E) > 0$, abbiamo
\begin{equation}
...
che abbiamo attribuito all'evento $H_j$.
