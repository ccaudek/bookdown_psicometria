...
Una regola decisionale comunemente usata, ma che la comunità statistica
...
La significatività statistica è convenzionalmente definita come un
$p$-valore inferiore a 0.05, relativo a qualche ipotesi nulla o valore
...
Parlando più in generale, una stima si dice "non statisticamente
significativa" se il suo valore osservato può essere ragionevolmente
spiegato con una semplice variazione casuale. 
...
Supponiamo di credere che una moneta sia equilibrata. La lanciamo 20 volte e
...
nulla del 50%. Di conseguenza, diciamo che il risultato non è "significativamente" diverso dal caso (ovvero, è __troppo simile__ al risultato predetto dall'ipotesi nulla).

## Un esempio motivante

Per introdurre in maggiore dettaglio il concetto di significatività statistica consideriamo una ricerca svolta da @mehr_melodies. La ricerca di @mehr_melodies
...
popolari, ad esempio. @mehr_melodies si sono chiesti se la musica sia
capace di trasmettere messaggi di tipo sociale anche in bambini molto
piccoli. Nello specifico, @mehr_melodies si sono chiesti se i bambini di
...
con una diversa melodia. @mehr_melodies hanno scoperto che, in effetti,
...
semplicemente la familiarità con la canzone.

### La domanda della ricerca

La domanda che @mehr_melodies si sono posti si chiama *domanda della
ricerca*. In psicologia, le domande della ricerca sono delle ipotesi che
...
la domanda che @mehr_melodies si sono posti è certamente una domanda
legittima nel contesto della ricerca psicologica.

In psicologia, le ipotesi della ricerca sono delle proposizioni che
...
già sappiamo rispetto al fenomeno psicologico considerato.
...
che i ricercatori possono fare, invece, è valutare delle *ipotesi
...
Nell'esperimento di @mehr_melodies, due settimane dopo la fase di
...
Dato che non è possibile valutare direttamente la domanda della ricerca
...
1.  Se i bambini non hanno alcuna preferenza nei confronti di uno dei
    ...
    popolazione) sarà uguale a $\mu = 0.5$, perché, in media, i tempi di
    fissazione per le due video-registazioni saranno uguali.

2.  Se @mehr_melodies hanno ragione, allora i bambini preferiranno
...
3.  Una terza possibilità è che i bambini siano maggiormente attratti da
    ...
    nell'ipotesi statistica $\mu < 0.5$.

Le tre ipotesi precedenti sono esempi di ipotesi statistiche. Sono
...
nell'esperiment di @mehr_melodies. Se consideriamo uno specifico
...
con i dati raccolti dagli sperimentatori perché @mehr_melodies hanno
misurato proprio questo aspetto, ovvero la media della proporzione del
tempo di fissazione rivolto al video "familiare".

### Domanda della ricerca e ipotesi statistiche

Ciò che la discussione precedente dovrebbe mettere in chiaro è che,
...
L'ipotesi statistica, invece, è: $\mu > 0.5$.

Ciò che dobbiamo avere ben chiaro è che i test vengono applicati alle
...
$\mu > 0.5$; ma questo non avrebbe nulla a che fare con la domanda della
ricerca.
...
provando la falsità della proposizione contraddittoria.
...
considerata. Questa è la logica del falsificazionismo di Popper.
...
ricerca (ovvero, $\mu > 0.5$).

In pratica, ciò che stiamo facendo qui è dividere tutti i possibili
valori di $\pi$ in due gruppi: quei valori che sono coerenti con
...
che questo modo di ragionare sia piuttosto strano.

### La similitudine del processo penale

Un test di ipotesi è stato paragonato ad un processo penale, ovvero al
...
sta cercando disperatamente di dimostrare che è essa è falsa.
...
impossibile), ma di ridurre gli errori al minimo.

A questo punto, dobbiamo precisare meglio cosa intendiamo per "errori". Iniziamo con il rendere esplicito quello che è ovvio: l'ipotesi nulla può essere vera o falsa, e il nostro test ci può condurre a rifiutare l'ipotesi nulla o a non rifiutarla. La decisione di rigettare o non rigettare l'ipotesi nulla ci espone dunque al rischio di
commettere uno di due tipi di errore, come indicato nella figura \@ref(fig:dueerrori). L'errore di I tipo, denotato con $\alpha$, è quello che commettiamo se rigettiamo l'ipotesi nulla quando essa è vera. L'errore di II tipo, denotato con $\beta$, è quello che commettiamo se accettiamo l'ipotesi nulla mentre invece è vera l'ipotesi
alternativa.

```{r dueerrori, fig.align = 'center', out.width = "80%", fig.cap = "Due tipi di errori.", echo = FALSE}
...
piccola soglia di probabilità prefissata.
...
Ritorniamo all'esempio relativo allo studio di @mehr_melodies. In questo
...
di età media pari a 5.6 mesi, @mehr_melodies hanno scoperto che, in
...
trovato la risposta alla domanda della ricerca.

### La variabilità campionaria

Nel caso dell'esperimento @mehr_melodies che stiamo discutendo,
...
ampiezza $n=32$ è una variabile aleatoria: in ciascun possibile campione
...
Risulta dunque chiaro che la nostra decisione rispetto ad $H_0$ non può
...
nulla sia vera o meno, in alcuni campioni la differenza $\bar{X} - \mu$
...
considerare unicamente la differenza $\bar{X} - \mu$.
...
È ovvio come calcolare la media delle proporzioni del tempo di
fissazione in un singolo campione. Il problema è che tale media varia da
campione a campione (fenomeno detto della *variabilità campionaria*).
L'approccio frequentista affronta il problema di giungere ad una
decisione rispetto ad $H_0$ *tenendo in considerazione* la variabilità
...
campione, ovvero $\bar{X}$) in tutti i infiniti possibili campioni di
ampiezza $n$ (nel nostro caso $n$ = 32).

È facile capire che, espresso in questi termini, il problema di
...
sufficienti per "condannare" l'ipotesi nulla, ovvero, per rigettarla.

In questa *reductio ad absurdum*, la "presunzione di innocenza" di $H_0$
corrisponde all'idea che dobbiamo assumere come vera l'ipotesi nulla,
*fino a prova contraria*. Nell'esempio che stiamo discutendo, assumere
...
campioni di ampiezza 32.

Per fare questo, @mehr_melodies utilizzano (implicitamente) ad un famoso
...
campione (ovvero, $n$ = 32). In altre parole, la teoria della
...
L'approccio frequentista fa uso di un insieme teoremi della teoria della
...
$H_0$ (regione di non rifiuto). Come facciamo a decidere quanto è grande
...
di $H_0$ come quell'intervallo di valori $\mathcal{G}_n$ a cui è
associata la probabilità $\alpha$, ovvero la probabilità di commettere
un errore di I tipo.

### Quando rifiutare l'ipotesi nulla

Supponiamo che la figura \@ref(fig:testipotesi1) rappresenti la distribuzione
campionaria della statistica test $\mathcal{G}_n$. Se i dati producono
la statistica test $\mathcal{G}_n^1$, non possiamo rifiutare l'ipotesi
nulla $H_0$. Se invece i dati producono $\mathcal{G}_n^2$ allora
possiamo rifiutare l'ipotesi nulla in favore dell'ipotesi alternativa.
Ci sono varie cose da notare.

1.  La regione di rifiuto è costituita da valori lontani dal centro
    della distribuzione campionaria della statistica test, la quale è
    stata costruita assumendo come vera $H_0$.

2.  La regione di rifiuto è situata nelle code della distribuzione.
    Vedremo in seguito anche degli esempi di regioni di rifiuto
    unilaterali.

3.  In questa discussione, l'ipotesi alternativa non è menzionata.
...
```{r testipotesi1, fig.align = 'center', out.width = "100%", fig.cap = "Distribuzione della statistica test condizionata all’ipotesi nulla $H_0$.", echo = FALSE}
...
-   Se l'ipotesi alternativa è $H_1: \theta < \theta_0$, allora le
    ...
    rigetto di $H_0$) sono contenute nell'intervallo $[-\infty, \theta_0]$ e l'intera regione di rifiuto $\mathcal{R}$ è collocata nella coda di sinistra della distribuzione.

-   Se l'ipotesi alternativa è $H_1: \theta > \theta_0$, allora le
    evidenze coerenti con l'ipotesi alternativa (e che portano al
    rigetto di $H_0$) sono contenute nell'intervallo $[\theta_0, \infty]$ e l'intera regione di rifiuto $\mathcal{R}$ è collocata
    nella coda di destra della distribuzione.

Si chiamano *valori critici* i valori che delimitano la regione di
...
ricade nella regione di rifiuto $\mathcal{R}$.

#### Problema {-}

Supponiamo che $f(\mathcal{G}_n \mid H_0) = \mathcal{N}(100, 15)$
...
critico che delimita la regione di rifiuto di $H_0$.

##### Soluzione {-}

Usando R la risposta è: `qnorm(0.95, 100, 15) = 124.7`. La distribuzione
$\mathcal{N}(100, 15)$ è mostrata nella figura figura \@ref(fig:testipotesi2). La regione di rifiuto è indicata dall'area ombreggiata.


```{r testipotesi2, fig.cap="Distribuzione campionaria con regione di rifiuto unilaterale destra."}
...
Supponiamo ora che $f(\mathcal{G}_n \mid H_0) = \mathcal{N}(100, 15)$
...
trovi il valore critico che delimita la regione di rifiuto di $H_0$.

##### Soluzione {-}

Usando R, la risposta è `qnorm(0.01, 100, 15)` = `r round(qnorm(0.01, 100, 15), 2)` -- si veda la figura \@ref(fig:testipotesi3).

```{r testipotesi3, fig.cap="Distribuzione campionaria con regione di rifiuto unilaterale sinistra."}
ggplot(data.frame(x = c(55, 145)), aes(x)) + 
  stat_function(fun = dnorm, args = list(mean = 100, sd = 15)) +
  stat_function(
    fun = dnorm, args = list(mean = 100, sd = 15),
...
In un terzo esempio, supponiamo che $f(\mathcal{G}_n \mid H_0) = \mathcal{N}(100, 15)$ descriva la distribuzione della statistica test $\mathcal{G}_n$. Supponiamo inoltre che la regione di rifiuto sia bilaterale e che il livello di
significatività sia $\alpha = 0.05$. Si trovino i valori critici che
delimitano la regione di rifiuto di $H_0$.

##### Soluzione {-}

Con la seguente istruzione `qnorm(c(0.025, 0.975), 100, 15)` troviamo i
valori $70.6$ e $129.4$ -- si veda la figura \@ref(fig:testipotesi4).

```{r testipotesi4, fig.cap="Distribuzione campionaria con regione di rifiuto bilaterale."}
ggplot(data.frame(x = c(55, 145)), aes(x)) + 
  stat_function(fun = dnorm, args = list(mean = 100, sd = 15)) +
  stat_function(
    fun = dnorm, args = list(mean = 100, sd = 15),
...
Ovviamente l'ipotesi a cui von Mises fa riferimento, la cui validità è
solo ipotetica, è l'ipotesi nulla.
...
Il *valore-p* rappresenta la probabilità di osservare un valore della
statistica test $\mathcal{G}_n$ pari a quello effettivamente osservato,
...
nulla. Tali concetti sono riassunti nella figura \@ref(fig:decisionestatistica).

```{r decisionestatistica, fig.align = 'center', out.width = "100%", fig.cap = "Relazione tra il valore-p e il livello di significatività alpha.", echo = FALSE}
...
in favore dell'alternativa quando $H_1$ è vera. Ma questi sono solo due
dei quattro esiti che, in principio, sono possibili, come indicato nella
tabella \@ref(fig:decisionestatistica) e corrispondono alle probabilità
indicate nella figura \@ref(fig:poterestatistico).

```{r poterestatistico, fig.align = 'center', out.width = "60%", fig.cap = "Probabilità dei due tipi di errori nel test di ipotesi statistiche.", echo = FALSE}
knitr::include_graphics(here::here("images", "potere_statistico.png"))
```

Possiamo pensare a $H_0$ come all'ipotesi che descrive l'evento "nulla
...
è vera l'ipotesi alternativa $H_1$. Nella figura \@ref(fig:altopoterestatistico), l'area ombreggiata sotto $f(\mathcal{G}_n \mid H_0)$ rappresenta il livello di significatività in un test unilaterale. Ricordiamo che il livello di significatività è la
probabilità di rifiutare falsamente l'ipotesi nulla quando essa è vera.
Invece, l'area sotto $f(\mathcal{G}_n \mid H_1)$ a sinistra della linea
...
regione di rifiuto di $H_0$ quando è vera $H_1$. Nella figura \@ref(fig:altopoterestatistico) la potenza del test è alta.

```{r altopoterestatistico, fig.align = 'center', out.width = "90%", fig.cap = "Probabilità dei due tipi di errori nel test di ipotesi statistiche.", echo = FALSE}
knitr::include_graphics(here::here("images", "alto_potere_statistico.png"))
```

Nella figura \@ref(fig:bassopoterestatistico), invece, la potenza del test è bassa.
Entrambi i test hanno lo stesso livello di significatività, ma se $f(\mathcal{G}_n \mid H_1)$ si sovrappone di molto con $f (\mathcal{G}_n \mid H_0)$, allora la potenza del test è bassa.

```{r bassopoterestatistico, fig.align = 'center', out.width = "90%", fig.cap = "Probabilità dei due tipi di errori nel test di ipotesi statistiche.", echo = FALSE}
