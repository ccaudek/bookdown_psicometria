# Strutture di dati {#chapter:strutture_dati_R}
...
-   Distinguere tra classi e modi degli oggetti di R.
-   Creare un data.frame in cui inserire i dati da analizzare.
-   Estrarre un sottoinsieme di dati da un data.frame.
...
<!-- ![Le strutture di dati più comuni in R  sono i vettori, le matrici, gli -->
<!-- array, le liste e i data.frame (figura riprodotta da Grolemund, -->
<!-- 2014).](images/R_data_structures.pdf) -->
...
essere vettori di tipi differenti (si veda la
Figura [1.1](#fig:R_data_structures){reference-type="ref"
reference="fig:R_data_structures"}). La funzione usata per generare un
...
Per fare un esempio, creiamo un data.frame che contenga tutte le informazioni di un mazzo di
carte da poker [@grolemund2014hands]. In tale data.frame, ciascuna riga
