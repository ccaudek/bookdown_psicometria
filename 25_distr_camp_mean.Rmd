# Distribuzione campionaria della media dei campioni {#chapter:distrcampmean}
...
incoraggianti: la media campionaria di `r round(mean(iq1), 2)` ci fornisce
...
```{definition, defsamplingdistr, name="Distribuzione campionaria"}
La distribuzione campionaria di una statistica basata su $n$
...
Il QI medio in questo campione risulta pari a `r round(mean(iq3), 2)`. Non sorprende che
...
In quest altro campione casuale il QI medio è `r round(mean(iq4), 2)`. Procediamo in
...
```{r histmeaniq, fig.cap="Istogramma della distribuzione delle medie dei punteggi del QI calcolate su 10000 campioni casuali di ampiezza $n=5$."}
...
trovato una media campionaria pari a `r round(mean(iq1), 2)`. Quello che impariamo dalla
...
singolo campione di numerosità $n$ = 5.

Si noti che la distribuzione campionaria, in generale, non è nota, poiché
dipende dalle caratteristiche della popolazione, non solo dai dati
osservati nel campione. In pratica, quindi, non possiamo conoscere la distribuzione
campionaria di una statistica; possiamo solo stimarla. Nella
discussione seguente mostreremo come sia possibile utilizzare la stima
della distribuzione campionaria per descrivere le proprietà statistiche
delle stime (ovvero, il grado di incertezza che è associato alle stime
che otteniamo).
...
```{r histmaxiq, fig.cap="Istogramma della distribuzione del QI massimo osservato in ciascun campione casuali di ampiezza $n=5$. Per creare la figura sono stati considerati 10000 campioni casuali."}
...
```{r tlctriangle1, fig.cap="Dimostrazione del Teorema del limite centrale. Consideriamo una popolazione che non segue la distribuzione normale. La distribuzione di tale popolazione è rappresentata dall'istogramma grigio."}
plot_one(1)
```
...
```{r tlctriangle2, fig.cap="Distribuzione campionaria di $\bar{X}$ per campioni casuali di ampiezza $n=2$ estratti dalla popolazione rappresentata nella figura 1.7."}
plot_one(2)
```
...
```{r tlctriangle4, fig.cap="Distribuzione campionaria di $\bar{X}$ per campioni casuali di ampiezza $n=4$ estratti dalla popolazione rappresentata nella figura 1.7."}
plot_one(4)
```

Già con $n=8$ l'approssimazione diventa molto buona, come indicato nella figura \@ref(fig:tlctriangle8). 

```{r tlctriangle8, fig.cap="Distribuzione campionaria di $\bar{X}$ per campioni casuali di ampiezza $n=8$ estratti dalla popolazione rappresentata nella figura 1.7."}
plot_one(8)
```
...
```{theorem, theotlc, name="Teorema del limite centrale."}
Siano $X_1, X_2, \dots$ variabili aleatorie i.i.d., tutte con lo stesso
