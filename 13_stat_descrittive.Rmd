...
Ci sono dunque `r length(unique(bysubj$esm_id))` soggetti i quali hanno ottenuto i valori sulla scala del BDI-II stampati di seguito (li presento ordinati dal più piccolo al più grande).
...
```{r hist1zetsche, out.width="80%", fig.cap="Istogramma per i valori BDI-II riportati da @zetsche_future_2019."}
p1 <- bysubj %>%
  ggplot(aes(x = bdi)) +
  geom_histogram(
    aes(y = ..density..),
...
```{r hist2zetsche, out.width="80%", fig.cap="Una rappresentazione più comune per l'istogramma dei valori BDI-II di @zetsche_future_2019 nella quale gli intervalli delle classi hanno ampiezze uguali."}
p2 <- bysubj %>%
  ggplot(aes(x = bdi)) +
  geom_histogram(
    aes(y = ..density..),
...
```{r zetschehist3, out.width="80%", fig.cap="Funzione di densità empirica per i valori BDI-II di @zetsche_future_2019."}
p3 <- bysubj %>% 
  ggplot(aes(x = bdi)) +
  geom_histogram(
    aes(y = ..density..), 
...
Nuovamente, se preferite un'introduzione veramente "soft" alla nozione di "tendenza  centrale" di una distribuzione statistica, vi rimando nuovamentew al [link](https://tinystats.github.io/teacups-giraffes-and-statistics/03_mean.html) che ho già suggerito in precedenza.
...
```{r violin-zetsche, out.width="95%", fig.cap = "Due versioni di un violin plot per i valori BDI-II di ciascuno dei due gruppi di soggetti esaminati da @zetsche_future_2019."}
library("patchwork")
...
```{remark, rem-quantile_interpolation}
...
```{r, zetsche-scatter, out.width="95%", fig.cap = "Associazione tra le variabili BDI-II e CES-D nello studio di @zetsche_future_2019. In rosso sono rappresentate le osservazioni del gruppo di controllo; in blu quelle dei pazienti."}
